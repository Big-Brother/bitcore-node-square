...
var BitcoinRPC = require('bitcoind-rpc-square');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-square. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-square. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-square. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that squared is undergoing a reindex. ' +
             ...
             'of bitcore-node-square services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype.getMNList = function(callback){
  ...
  //DEACTIVATED because of slow response from squared
