...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bitcore-node-square.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node-square/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed bitcore-node-square.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitcore-node-square.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bitcore-node-square services directory
