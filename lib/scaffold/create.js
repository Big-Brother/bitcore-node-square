...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'bitcore-lib-square': '^' + bitcore.version,
    'bitcore-node-square': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-square.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitcore-node-square directory and configuration
