...
To develop bitcore-node-square:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-square.git
...
We now will setup symlinks in `bitcore-node-square` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-square
ln -s ~/bitcoind-rpc-square
```
...
cd devnode
mkdir node_modules
touch bitcore-node-square.json
touch package.json
```

Edit `bitcore-node-square.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-square/bin/bitcore-node-square start
```