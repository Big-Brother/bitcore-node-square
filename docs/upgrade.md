...
`bitcore-node-square@2.1.1` to `bitcore-node-square@3.0.0`
...
```json
{
  ...
  "servicesConfig": {
    "bitcoind": {
      "spawn": {
        "datadir": "/home/<username>/.bitcoin",
        "exec": "/home/<username>/bitcore-node-square/bin/bitcoind"
      }
    }
  }
}
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `bitcore-node-square.db` directory with the old indexes.
